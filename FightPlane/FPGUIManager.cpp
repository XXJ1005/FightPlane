...
FPGUIManager *FPGUIManager::m_Manager = nullptr;
...
void FPGUIManager::InitStartGameGUI() {
	// 创建开始游戏按钮
	RECT windowsRect;
	GetClientRect(myGLRender->getHwnd(), &windowsRect);
...
void FPGUIManager::InitPlayGameGUI() {
	...
	FPGUIManager::GetInstance()->m_ScoreText = new FPText(-0.1, -0.3, 64, color);
...
void FPGUIManager::LButtonDownHandle(WPARAM wParam, LPARAM lParam) {
	...
	int y = HIWORD(lParam);
	RECT rect;
	GetClientRect(myGLRender->getHwnd(), &rect);
...
void FPText::Render(const char* text) {
	...
	RECT clientRect;
	GetClientRect(myGLRender->getHwnd(), &clientRect);
	...
	// 计字体大小对应像素值
	HDC hDC = GetDC(myGLRender->getHwnd());
...
void overGameBtnUpBlock(ButtonType type) {
	if (type == ButtonType::LButton) {
		// 退出游戏程序
		PostMessage(myGLRender->getHwnd(), WM_CLOSE, 0, 0);
	}
}
